...
def main(base_dir):
    ...
    for root, dirs, _ in os.walk(base_dir):
        ...
        if 'logs' in dirs:
            ...
            # 遍历 logs 文件夹中的文件
            for log_root, _, files in os.walk(logs_path):
                for file in files:
                    # 只处理 .log 或 .gz 文件
                    if file.endswith('.log') or file.endswith('.gz'):
                        file_list.append(os.path.join(log_root, file))
    ...
    for file_path in file_list:
        ...
        if process_file(file_path, initial_date):
            ...
            archived_path = os.path.join(archived_dir, os.path.relpath(file_path, base_dir))
            os.makedirs(os.path.dirname(archived_path), exist_ok=True)
            shutil.move(file_path, archived_path)
        else:
            error_path = os.path.join(error_dir, os.path.relpath(file_path, base_dir))
...
if __name__ == '__main__':
    # 指定要扫描的基目录
    base_directory = r"D:\Minecraft\.minecraft\versions"
    main(base_directory)